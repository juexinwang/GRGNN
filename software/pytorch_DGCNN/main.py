...
def loop_dataset(g_list, classifier, sample_idxes, optimizer=None, bsize=cmd_args.batch_size):
    ...
    for pos in pbar:
        ...
        if classifier.regression:
            pred, mae, loss, mask = classifier(batch_graph)
            all_scores.append(pred.cpu().detach())  # for binary classification
        else:
            logits, loss, acc, mask = classifier(batch_graph)
            all_scores.append(logits[:, 1].cpu().detach())  # for binary classification
            prob_scores.append(logits[:, 0].cpu().detach()-logits[:, 1].cpu().detach())
        ...
        results = mask.data.cpu().numpy()
        idx=np.where(results)[0]
        all_results.extend(np.take(selected_idx, idx))
    ...
    prob_scores = torch.cat(prob_scores).cpu().numpy()
    ...
    return avg_loss, all_results, prob_scores
